...
const pillars = [
  {
    ...
    copy: "Every pack carries a unique serial hashed into its origin block.",
  },
  {
    icon: RouteIcon,
    title: "Custody you can replay",
    copy: "Plant, distributor, pharmacy — each handoff appends a new block.",
  },
  {
    icon: Lock,
    title: "Tamper-evident",
    copy: "Blocks link by hash, so a rewritten record breaks the chain visibly.",
  },
];
...
function Home() {
  return (
    <SiteShell>
      <section className="pb-16 text-center sm:text-left">
        ...
        <h1 className="mx-auto mt-6 max-w-3xl text-4xl font-bold leading-[1.06] tracking-tight sm:mx-0 sm:text-6xl">
      ...
      <div className="grid gap-6 sm:grid-cols-3">
        {actions.map(({ to, icon: Icon, label, copy }) => (
          <Link key={to} to={to} className="group">
            <Panel className="h-full transition-shadow group-hover:shadow-lift">
              <span className="flex size-11 items-center justify-center rounded-2xl bg-primary-soft text-primary">
                <Icon className="size-5" />
              </span>
              <h2 className="mt-5 text-lg font-semibold tracking-tight">{label}</h2>
